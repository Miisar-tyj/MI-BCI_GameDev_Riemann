...
def character_importer(cols, rows, *path):
	...
	new_dict = {}
	for row, direction in enumerate(('idle', 'walk', 'placeholder')):
		new_dict[direction] = [frame_dict[(col, row)] for col in range(cols)]
		new_dict[f'{direction}_idle'] = [frame_dict[(0, row)]]    #useless cuz i dont have this
	return new_dict
...
def all_character_import(*path):
	...
	for _, __, image_names in walk(join(*path)):
		for image in image_names:
			...
			new_dict[image_name] = character_importer(4,3,*path, image_name)
	return new_dict
